    ...
    with tab1:
        ...
        if not pareto_df.empty:
            base = alt.Chart(pareto_df).encode(
                ...
                tooltip=['provider', 'policy', 'avg_latency', 'avg_cost', 'is_pareto']
            )
...
with bottom_left_cell:
    ...
    # Row 1: Top Metrics (Custom Cards for alignment and spacing)
    row1 = st.columns([0.8, 1.2])
    
    with row1[0]:
        st.markdown(f"""
            <div style="background-color: #1f2833; padding: 12px; border-radius: 8px; border: 1px solid #45a29e; min-height: 105px; display: flex; flex-direction: column; justify-content: flex-start; overflow: hidden; margin-top: 2px;">
                <p style="font-weight: 400; font-size: 0.8rem; color: #c5c6c7; margin: 0; line-height: 1.2;">Requests Analyzed</p>
                <div style="height: 10px;"></div>
                <p style="font-size: 1.6rem; font-weight: 400; color: #ffffff; margin: 0; line-height: 1;">{total_reqs:,}</p>
            </div>
        """, unsafe_allow_html=True)
        
    with row1[1]:
        st.markdown(f"""
            <div style="background-color: #1f2833; padding: 12px; border-radius: 8px; border: 1px solid #45a29e; min-height: 105px; display: flex; flex-direction: column; justify-content: flex-start; overflow: hidden;">
                <p style="font-weight: 400; font-size: 0.8rem; color: #c5c6c7; margin: 0; line-height: 1.2;">P95 Latency</p>
                <div style="height: 8px;"></div>
                <p style="font-size: 1.6rem; font-weight: 400; color: #ffffff; margin: 0; line-height: 1;">{p95_latency:.0f}ms</p>
                <p style="font-size: 0.75rem; color: #888; margin-top: 4px;">↑ SLA: 1500ms</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    # Row 2: Bottom Metrics (Custom Cards for alignment and color)
    row2 = st.columns([0.8, 1.2])
    
    with row2[0]:
        st.markdown(f"""
            <div style="background-color: #1f2833; padding: 12px; border-radius: 8px; border: 1px solid #45a29e; min-height: 105px; display: flex; flex-direction: column; justify-content: flex-start; overflow: hidden;">
                <p style="font-weight: 400; font-size: 0.8rem; color: #c5c6c7; margin: 0; line-height: 1.2; white-space: nowrap;">Success Rate</p>
                <div style="height: 10px;"></div>
                <p style="font-size: 1.6rem; font-weight: 400; color: {health_color}; margin: 0; line-height: 1; white-space: nowrap;">{success_rate:.1f}%</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    with row2[1]:
        st.markdown(f"""
            <div style="background-color: #1f2833; padding: 12px; border-radius: 8px; border: 1px solid #45a29e; min-height: 105px; display: flex; flex-direction: column; justify-content: flex-start; overflow: hidden;">
                <p style="font-weight: 400; font-size: 0.8rem; color: #c5c6c7; margin: 0; line-height: 1.2; white-space: nowrap;">Total Spend</p>
                <div style="height: 10px;"></div>
                <p style="font-size: 1.6rem; font-weight: 400; color: #ffffff; margin: 0; line-height: 1; white-space: nowrap;">${total_spend:.2f}</p>
            </div>
        """, unsafe_allow_html=True)
    
    # Add bottom margin to prevent cards from touching the container border
        ...
        for i, provider in enumerate(providers):
            ...
            with grid_cols[i % NUM_COLS]:
                with st.container(border=True):
                    ...
                    if not p_data.empty:
                        # Smooth moving average for aesthetic area chart
                        # Aggregate to 1s window assuming rapid requests
                        ...
                        mc = st.columns([1.3, 1.0, 0.7])
                        mc[0].metric("Avg Latency", f"{avg_lat:.0f}ms")
                        mc[1].metric("Avg Cost", f"{avg_cost:.2f}¢")
                        mc[2].metric("Errors", f"{fails}")
                    else:
                        st.info("No data")
